...
class SOCKSv5Protocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        if self.state == self.ST_READ_METHODS:
            ...
            logging.error("Client sent data before receiving response")
            self.transport.loseConnection()
        else:
            logging.error("Invalid state in SOCKS5 Server: '%d'" % self.state)
            self.transport.loseConnection()
    ...
    def processMethodSelect(self):
        ...
        if ver != _SOCKS_VERSION:
            logging.error("Method Select Invalid SOCKS version: '%d'" % ver)
            self.transport.loseConnection()
            return
        if nmethods == 0:
            logging.error("No Authentication method(s) present")
            self.transport.loseConnection()
            return
        if len(msg) < nmethods:
            return
        ...
        if self.authMethod == _SOCKS_AUTH_NO_ACCEPTABLE_METHODS:
            logging.error("No Acceptable Authentication Methods")
            self.authMethod = _SOCKS_AUTH_NO_ACCEPTABLE_METHODS

        # Ensure there is no trailing garbage
        if len(msg) > 0:
            logging.error("Peer sent trailing garbage after method select")
            self.transport.loseConnection()
            return
        self.buf.clear()
    ...
    def processAuthentication(self):
        ...
        if self.authMethod in self.AUTH_METHOD_VTABLE:
            ...
            # Should *NEVER* happen
            logging.error("Peer sent data when we failed to negotiate auth")
            self.buf.clear()
            self.transport.loseConnection()
    ...
    def processRfc1929Request(self):
        ...
        if ver != _SOCKS_RFC1929_VER:
            logging.error("Invalid RFC1929 version: '%d'" % ver)
            self.sendRfc1929Reply(False)
            return
        if ulen == 0:
            logging.error("Username length is 0")
            self.sendRfc1929Reply(False)
            return
        ...
        if len(msg) < plen:
            return
        if plen == 0:
            logging.error("Password length is 0")
            self.sendRfc1929Reply(False)
            return
        passwd = msg.get(plen)
        ...
        # Ensure there is no trailing garbage
        if len(msg) > 0:
            logging.error("Peer sent trailing garbage after RFC1929 auth")
            self.transport.loseConnection()
            return
        self.buf.clear()
    ...
    def processRequest(self):
        ...
        atyp = msg.get_uint8()
        if ver != _SOCKS_VERSION:
            logging.error("ProcessRequest Invalid SOCKS version: '%d'" % ver)
            self.sendReply(SOCKSv5Reply.GeneralFailure)
            return
        if cmd not in self.ACCEPTABLE_CMDS:
            logging.error("Invalid SOCKS command: '%d'" % cmd)
            self.sendReply(SOCKSv5Reply.CommandNotSupported)
            return
        if rsv != _SOCKS_RSV:
            logging.error("Invalid SOCKS RSV: '%d'" % rsv)
            self.sendReply(SOCKSv5Reply.GeneralFailure)
            return
        ...
        if atyp == _SOCKS_ATYP_IP_V4:
            ...
            alen = msg.get_uint8()
            if alen == 0:
                logging.error("Domain name length is 0")
                self.sendReply(SOCKSv5Reply.GeneralFailure)
                return
            if len(msg) < alen:
                return
            ...
            logging.error("Invalid SOCKS address type: '%d'" % atyp)
        ...
        # Ensure there is no trailing garbage
        if len(msg) > 0:
            logging.error("Peer sent trailing garbage after request")
            self.transport.loseConnection()
            return
        self.buf.clear()
        ...
        if cmd == _SOCKS_CMD_CONNECT:
            ...
            # Should *NEVER* happen
            logging.error("Unimplemented command received")
            self.transport.loseConnection()
    ...
    def connectClass(self, addr, port, klass, *args):
        return protocol.ClientCreator(self.reactor, SOCKSv5Outgoing, self).connectTCP(addr, port)

    def handleCmdConnectFailure(self, failure):
        logging.error("CMD CONNECT: %s" % failure.getErrorMessage())
...
class SOCKSv5Factory(protocol.Factory):
    ...
    def startFactory(self):
        logging.debug("%s: Starting up SOCKS server factory." % self.name)

    def buildProtocol(self, addr):
        logging.debug("%s: New connection." % self.name)
