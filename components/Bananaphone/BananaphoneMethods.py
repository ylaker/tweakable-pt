...
import os, sys, time, readline, logging
...
def removeIncompleteSpaces ( model, bits ):
    ...
    logging.debug( "%s states - %s leading to complete spaces = %s to drop"
...
def markov ( tokenize, hash, bits, corpusFilename, order=1, abridged=None ):
    ...
    logging.debug( "%s states (%s tokens, %s unique, order=%s)" % ( len(model), len(corpusTokens), len( set(corpusTokens) ), order ) )
    ...
    if abridged == "--abridged":
        ...
        logging.debug( "%s states which lead to incomplete hash spaces will not be used." % ( incompleteStates, ) )
    ...
    def encodeFactory():
        ...
        def encode ( value ):
            ...
            if not stats.total % 10**5:
                logging.debug( "%s words encoded, %s%% adhering to model" % ( stats.total, (100.0* stats.adhered / stats.total ) ) )
...
def random ( tokenize, hash, bits, corpusFilename, shortest=False ):
    ...
    logging.debug( "built weighted random model from %s tokens (%s unique)" % ( len(corpusTokens), len(set(corpusTokens)) ) )
...
def hammertime_decoder ( target ):
    ...
    while True:
        ...
        logging.debug ("Frame type %s length %s" % (frameType, frameLength))
...
def httpd_chooser ( encodingSpec   = 'words,sha1,12',
    ...
    truncatedHash = truncateHash( hash, bits )
    corpusTokens  = list( tokenize < readTextFile( corpusFilename ) )
    logging.debug( "Building model from %s tokens" % (len(corpusTokens),) )
...
Select alternates:
    ...
    logging.debug( "Listening on port %s" % (port,) )
...
def tab_composer ( 
    ...
    tokenize, hash, bits = parseEncodingSpec( encodingSpec )
    truncatedHash = truncateHash( hash, bits )
    logging.debug( "Tokenizing corpus" )
    corpusTokens  = list( tokenize < readTextFile( corpusFilename ) )
    logging.debug( "Building random model" )
    randomModel   = buildWeightedRandomModel( corpusTokens, truncatedHash )
    percentFull   = getPercentFull( randomModel, bits )
    assert percentFull == 100, "not enough tokens for %s-bit hashing (%s%% there)" % (bits, percentFull)
...
#    markovModel   = removeIncompleteSpaces( markovModel, bits ) #BUG
    logging.debug( "Scaling input data" )
